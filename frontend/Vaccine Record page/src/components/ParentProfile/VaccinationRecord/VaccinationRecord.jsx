...
const VaccinationRecord = () => {
    ...
    const renderChildInfo = (child) => {
        const administeredVaccines = child.vaccinationHistory.filter(vaccine => vaccine.vaccinationStatus === 'Administered').length;
        ...
        return (
            <div className="child-profile" key={child._id}>
                <Typography variant="h5" gutterBottom>
                    <p><strong>Full Name:</strong>{child.name}</p>
                    <p><strong>Gender:</strong> {child.gender}</p>
                    <p><strong>Age:</strong> {child.age} years</p>
                </Typography>
                <Typography variant="subtitle1">
                    <p><strong>Parent:</strong> {child.relationshipWithParent},</p>
                    <p><strong>Address :</strong> {child.address.city},</p>
                </Typography>
                ...
                <TableContainer component={Paper}>
                    <Table>
                      ...
                      Date: ${vaccine.dateAdministered ? new Date(vaccine.dateAdministered).toDateString() : 'Not Administered'}
                    ...
                    `)}>
                                    ...
                                    <TableCell style={{ color: vaccine.vaccinationStatus === 'Administered' ? 'green' : 'red' }}>
                                        {vaccine.vaccinationStatus}
                                    </TableCell>
                                    <TableCell>
                                        {vaccine.vaccinationStatus === 'Administered' ? (
                                            <button className="show-certificate" onClick={() => alert(`Showing certificate for ${vaccine.vaccineName}`)}>
                                                Show Certificate
                                            </button>
                                        ) : (
                                            <button className="book-appointment" onClick={() => alert(`Booking appointment for ${vaccine.vaccineName}`)}>
                                                Book Appointment
                                            </button>
                                        )}
                                    </TableCell>
